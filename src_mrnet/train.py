...
def train(
    ...
    backbone_params = (
        list(model.axial_net.parameters())
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {"params": classifier_params, "lr": learning_rate * classifier_lr_mult},
        ],
    ...
    if ckpt_path:
        ...
        ckpt = torch.load(ckpt_path, map_location=(None if use_gpu else "cpu"))
        ...
        if model_path:
            ...
            state = torch.load(model_path, map_location=(None if use_gpu else "cpu"))
    ...
    if not log_path.exists():
        with open(log_path, "w") as f:
            f.write("epoch,train_loss,train_auc,val_loss,val_auc,backbone_lr,classifier_lr\n")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        with open(log_path, "a") as f:
            f.write(
                f"{epoch+1},{train_loss:.6f},{train_auc:.6f},"
                f"{val_loss:.6f},{val_auc:.6f},{backbone_lr:.8f},{classifier_lr:.8f}\n"
            )
...
def get_parser():
    ...
    parser.add_argument("--classifier_lr_mult", type=float, default=20.0)
...
if __name__ == "__main__":
    ...
    if args.task != "abnormal" and args.abnormal_model is None:
        raise ValueError("Cần cung cấp --abnormal_model cho task 'acl' hoặc 'meniscus'")
